...
class Sphere(Model):
	def __init__(self, r: float = 1.0, divs1: float = 12, divs2: float = 12):
		super().__init__()
		self.buildSphere(r, divs1, divs2)

	def generateSpherePoint(self, r: float, alpha: float, beta: float) -> vec4:
		alpha = radians(alpha)
		beta = radians(beta)
		return vec4(
	...
	def generateSphereFace(
		...
		vertices[0] = self.generateSpherePoint(r, alpha, beta)
		vertices[1] = self.generateSpherePoint(r, alpha + step_alpha, beta)
		vertices[2] = self.generateSpherePoint(r, alpha + step_alpha, beta + step_beta)
		vertices[3] = self.generateSpherePoint(r, alpha, beta + step_beta)
	...
	def buildSphere(self, r: float, mainDivs: float, tubeDivs: float):
		face = [vec4() for _ in range(4)]
		faceVertexNormals = [vec4() for _ in range(4)]
		normal = vec4()
		...
		for alpha in range(round(tubeDivs)):
			for beta in range(round(mainDivs)):
				face, faceVertexNormals, normal = self.generateSphereFace(
					face,
					faceVertexNormals,
					normal,
					r,
					alpha * mult_alpha - 90.0,
					beta * mult_beta,
					mult_alpha,
					mult_beta,
				)
